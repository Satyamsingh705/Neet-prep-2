...
function TabButton({ label, active, onClick }: { label: string, active: boolean, onClick: () => void }) {
    return (
        <button 
            onClick={onClick}
            className={`px-6 py-2 text-xs font-black rounded-xl transition-all tracking-wide ${
                active ? "bg-white text-zinc-900 shadow-md border border-zinc-200/50" : "text-zinc-500 hover:text-zinc-800 hover:bg-zinc-200/30"
            }`}
        >
            {label}
        </button>
...
function TimeBlock({ value, label }: { value: string, label: string }) {
    return (
        <div className="flex flex-col items-center min-w-[60px]">
            <div className="text-4xl font-black tabular-nums tracking-tight text-white">{value}</div>
        ...
        <div className="group relative flex flex-col lg:flex-row lg:items-center justify-between gap-8 p-8 hover:bg-zinc-50/80 transition-all duration-300">
            ...
            <div className="flex flex-row lg:flex-col items-center lg:items-end justify-between lg:justify-center gap-5 w-full lg:w-auto pt-4 lg:pt-0 border-t lg:border-t-0 border-zinc-100">
                ...
                <div className="flex gap-3">
                    {isLive ? (
                        <Link href={`/live-arena/${test.id}`} className="h-12 px-8 flex items-center justify-center rounded-[18px] bg-emerald-500 text-white text-[11px] font-black uppercase tracking-[0.15em] hover:bg-emerald-600 hover:scale-[1.05] transition-all shadow-xl shadow-emerald-500/30 active:scale-95">
                            Enter Arena
                        </Link>
                    ) : isUpcoming ? (
                        <button 
                            ...
                            disabled={isPending}
                            className={`h-12 px-8 flex items-center justify-center rounded-[18px] text-[11px] font-black uppercase tracking-[0.15em] transition-all active:scale-95 ${
                                test.isRegistered 
